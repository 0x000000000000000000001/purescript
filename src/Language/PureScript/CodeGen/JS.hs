...
moduleToJs
  ...
  -> Maybe PSString
  -> m AST.Module
moduleToJs (Module _ coms mn _ imps exps reExps foreigns decls _ _) foreignInclude =
...
moduleBindToJs mn = bindToJs
  ...
  bindToJs (NonRec (_, _, _, Just IsTypeClassConstructor, _) _ _) = pure []
  ...
  nonRecToJS a i e@(extractAnn -> (_, com, _, _, _)) | not (null com) = do
  ...
  nonRecToJS (ss, _, _, _, _) ident val = do
    js <- valueToJs val
  ...
  guessEffects = \case
    ...
    App (_, _, _, Just IsSyntheticApp, _) _ _ -> NoEffects
    _                                   -> UnknownEffects
  ...
  valueToJs e =
    let (ss, _, _, _, _) = extractAnn e in
    withPos ss =<< valueToJs' e

  valueToJs' :: Expr Ann -> m AST
  valueToJs' (Literal (pos, _, _, _, _) l) =
  ...
  valueToJs' e@App{} = do
    ...
    case f of
      Var (_, _, _, Just IsNewtype, _) _ ->
        return (headDef (internalError "Newtype constructor without constructor name") args')
      Var (_, _, _, Just (IsConstructor _ fields), _) name | length args == length fields ->
  ...
  valueToJs' (Var (_, _, _, Just IsForeign, _) qi@(Qualified (ByModuleName mn') ident)) =
    return $ if mn' == mn
             then foreignIdent ident
             else varToJs qi
  valueToJs' (Var (_, _, _, Just IsForeign, _) ident) =
  ...
  valueToJs' (Case (ss, _, _, _, _) values binders) = do
  ...
  valueToJs' (Constructor (_, _, _, Just IsNewtype, _) _ ctor _) =
  ...
  binderToJs s done binder =
    let (ss, _, _, _, _) = extractBinderAnn binder in
  ...
  binderToJs' varName done (ConstructorBinder (_, _, _, Just IsNewtype, _) _ _ [b]) =
    binderToJs varName done b
  binderToJs' varName done (ConstructorBinder (_, _, _, Just (IsConstructor ctorType fields), _) _ ctor bs) = do
